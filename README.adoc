...
Run application and open GraphiQL at http://localhost:8080/graphiql?path=/graphql

== What is GraphiQL?
GraphiQL is the GraphQL integrated development environment (IDE). It offers syntax highlighting, intellisense autocompletion, automatic documentation, and much more.
...
Add the following to a test file `UserControllerTest.java`. Save this file in a location within the `src/test/java/com/example/graphqlserver/` folder.

[source,java]
----
include::complete/src/test/java/com/example/graphqlserver/UserControllerTest.java[]
----
...
Add this query in a `firstUser.graphql` file located in `src/test/resources/graphql-test`.

[source,graphql]
----
include::complete/src/test/resources/graphql-test/firstUser.graphql[]
----
